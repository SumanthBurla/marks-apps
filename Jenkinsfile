...
pipeline {
    ...
    stages {
        ...
        stage('Push to hub'){
            steps{
                pushImage()
                echo "---------${env.IMAGE_NAME}:v${env.BUILD_ID}.0.0--------"
            }
        }
        ...
        stage('trigger runApp build'){
            steps{
                script{
                    build job: 'runApp',
                    parameters:[
                        [ $class: 'StringParameterValue', name:'imageName_fromBuild', value:"${env.IMAGE_NAME}:v${env.BUILD_ID}.0.0"]
                    ]
                }
            }
        }
...
def buildImage(){
    sh('docker build -t $IMAGE_NAME:v$BUILD_ID.0.0 .')
    echo "Build complete..."
    sh('docker images')
}
def runApp(){
    sh('docker run -d -p 8083:5000 $IMAGE_NAME:v$BUILD_ID.0.0')
}

def pushImage(){
    sh('docker push $IMAGE_NAME:v$BUILD_ID.0.0')
